...
% Cria variáveis para armazenar os mínimos, máximos e médias das taxas de acerto.
range_ = 0:10:200;
...
% Se o intervalo definido possuir mais de um valor, salvar os dados em um arquivo,
% caso contrário, apenas exibir no terminal.
savedata = false;
if length(range_) > 1
...
for n = range_
    ...
    % Calcula as taxas de acerto com K-fold da MLP com n neurônios na camada oculta.
    [success, total] = mlp10fold(Xraw, Yraw, [n]);
    ...
    % Salva ou apenas imprime os dados no terminal, de acordo com a variável savedata.
    if savedata
        data = [range_; min_; max_; mean_; min_total; max_total; max_total];
        csvwrite('mlpdata.csv', data);
    else
        n
        success
        min(success)
